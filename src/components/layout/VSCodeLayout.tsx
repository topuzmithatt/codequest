...
function HeartsIndicator({ hearts, heartsLastFill }: { hearts: number; heartsLastFill: Date | null }) {
  ...
  return (
    <div className="relative group flex items-center gap-0.5">
      ...
      <span className="ml-1.5 opacity-75">{hearts}/5</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Activity Bar */}
        <div
          className="flex flex-col items-center py-2 w-12 shrink-0 gap-0.5"
          style={{ background: "#333333", borderRight: "1px solid #1e1e1e" }}
        >
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {item.icon}
                <span
                  className="absolute left-14 px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition-opacity"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {streak > 0 && <span className="flex items-center gap-1">🔥<span className="opacity-90">{streak} gün</span></span>}
          {weeklyRank != null && <span className="opacity-70">#{weeklyRank} bu hafta</span>}
        </div>
